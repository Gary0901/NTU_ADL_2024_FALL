...
def main():
    ...
    training_args = Seq2SeqTrainingArguments(
        output_dir="./model_result/greedy_strategy",
    ...
    def predict_greedy(trainer):
        predictions = trainer.predict(valid_object, num_beams=1)# 設置 num_beams=1 for greedy search
        return predictions

    print(f"Model is on device: {model.device}")
    torch.cuda.empty_cache()

    # 使用 greedy search 進行預測
    predict_greedy(trainer)
    trainer.train()
