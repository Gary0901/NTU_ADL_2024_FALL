...
def load_custom_dataset(context_file,data_file):
    ...
    processed_data = []
    for item in data:
        context = contexts[int(item['relevant'])]
...
def parse_args():
    ...
    parser.add_argument(
    "--context_file", type=str, default="context.json", help="A json file containing the context paragraphs."
    )   
    parser.add_argument(
...
def main():
    ...
    if args.dataset_name is not None:
        raise ValueError("This script doesn't support loading datasets from the hub. Please use custom files.")
    else:
        raw_datasets = {}
        if args.train_file is not None:
            raw_datasets["train"] = load_custom_dataset(args.context_file, args.train_file)
        if args.validation_file is not None:
            raw_datasets["validation"] = load_custom_dataset(args.context_file, args.validation_file)
        if args.test_file is not None:
            raw_datasets["test"] = load_custom_dataset(args.context_file, args.test_file)

        if not raw_datasets:
            raise ValueError("No valid input files were specified.")
    ...
    """ logger.info(f"Epoch {epoch}: Average train loss: {avg_train_loss:.4f}, Exact Match: {eval_metric['exact_match']:.2f}") """
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            ...
            if completed_steps >= args.max_train_steps:
                break
        # 紀錄每個epoch的平均loss
        avg_train_loss = total_loss / len(active_dataloader)
        train_losses.append(avg_train_loss)

        # 在每個epoch結束後進行評估
        model.eval()
        eval_metric = evaluate_model(model, eval_dataloader, accelerator)
        eval_exact_matches.append(eval_metric['exact_match'])
    ...
    if accelerator.is_main_process:
        ...
        np.save(os.path.join(args.output_dir, 'eval_exact_matches.npy'), np.array(eval_exact_matches))

        # 调用plot.py来绘制图表
        import subprocess
        subprocess.run(["python", "plot.py", "--output_dir", args.output_dir])
